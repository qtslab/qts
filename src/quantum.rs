...
pub fn update_qubits(best_sol: Solution, worst_sol: Solution, qubits: &mut Qubits) -> i32 {
    ...
    for i in 0..qubits.len() {
        ...
        qubits[i].alpha = qubits[i].alpha * (theta * mod_signal as f64).cos() - qubits[i].beta * (theta * mod_signal as f64).sin();
        qubits[i].beta = qubits[i].beta * (theta * mod_signal as f64).sin() + qubits[i].alpha * (theta * mod_signal as f64).cos();
    }

    0
}

pub fn update_qubits_with_angle(best_sol: Solution, worst_sol: Solution, qubits: &mut Qubits, angle: f64) -> i32 {
